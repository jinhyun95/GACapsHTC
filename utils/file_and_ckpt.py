...
def make_label_sequences(hierarchy, label_ids):
    # MAKE LIST OF TOPICS THAT LEAD TO LEAVES
    # [[LEAF 1, TOPIC 1, TOPIC 2, ..., HIGHEST TOPIC], [...], ...]
...
def load_checkpoint(checkpoint, model, optimizer, mode='train'):
    ...
    for k in list(checkpoint['state_dict'].keys()):
        if 'text_encoder_cnns' in k:
            checkpoint['state_dict']['text_encoder_cnn.' + k.split('.')[-1]] = checkpoint['state_dict'][k]
            del checkpoint['state_dict'][k]
        elif 'information_aggregation.classifiers' in k:
            checkpoint['state_dict'][k.replace('classifiers', 'classifier')] = checkpoint['state_dict'][k]
            del checkpoint['state_dict'][k]
    ...
    if mode == 'train':
        ...
        try:
            ...
            return 0, {'micro-f1': 0., 'macro-f1': 0.}
    else:
